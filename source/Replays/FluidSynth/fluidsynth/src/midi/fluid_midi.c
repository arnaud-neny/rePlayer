...
fluid_midi_file_read_mthd(fluid_midi_file *mf)
{
    ...
    mf->type = mthd[9];
    if(!(mf->type == 0 || mf->type == 1 || mf->type == 2)) // rePlayer
    {
        FLUID_LOG(FLUID_ERR,
...
fluid_midi_file_read_event(fluid_midi_file *mf, fluid_track_t *track)
{
    ...
    else if(status == MIDI_META_EVENT)      /* meta events */
    {
        ...
        case MIDI_COPYRIGHT:
            ...
            fluid_track_set_name(track, (char*)metadata); // rePlayer
            break;
...
// rePlayer begin
    ...
    char* oldname = track->name;
    size_t oldlen = 0;
    if (oldname)
        oldlen = FLUID_STRLEN(oldname) + 1;

    track->name = FLUID_MALLOC(len + oldlen + 1);
    ...
    if (oldname)
    {
        FLUID_STRCPY(track->name, oldname);
        track->name[oldlen - 1] = '\n';
        FLUID_FREE(oldname);
        FLUID_STRCPY(track->name + oldlen, name);
    }
    else
    {
