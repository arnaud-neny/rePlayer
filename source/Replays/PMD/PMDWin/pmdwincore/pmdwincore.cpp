...
PMDWIN* PMDWIN::Create(core::io::Stream* stream)
{
	return new PMDWIN(stream);
}
...
//	コンストラクタ・デストラクタ
//-----------------------------------------------------------------------------
PMDWIN::PMDWIN(core::io::Stream* stream)
{
	fileio = new FileIO(stream);
	fileio->AddRef();
	pfileio = fileio;
	pfileio->AddRef();
...
//	音量LFO用サブ
//-----------------------------------------------------------------------------
void PMDWIN::fmlfo_sub(QQ *qq, int al, int bl, uint8_t *vol_tbl)
{
...
//	PSGﾘｽﾞﾑ ON
//=============================================================================
uint8_t * PMDWIN::rhythmon(QQ *qq, uint8_t *bx, int al, int *result)
{
	int		cl, dl, bx_;
...
//		output	bx	address
//=============================================================================
uint8_t * PMDWIN::toneadr_calc(QQ *qq, int dl)
{
	uint8_t *bx;
...
//=============================================================================
int PMDWIN::music_load3(uint8_t *musdata, int size, TCHAR *current_dir)
{
...
//	 ppc_load その３（メモリから）
//=============================================================================
int PMDWIN::ppc_load3(uint8_t *pcmdata, int size)
{
	int			i;
...
int WINAPI PMDWIN::getpcmdata(int16_t *buf, int nsamples)
{
	...
	do {
		if(nsamples - copysamples <= us2) {
			...
			copysamples += us2;
			pos2 = (char *)wavbuf2;
			auto oldstatus2 = open_work.status2;
			if (oldstatus2 == -1)
			{
				us2 = 0;
				return copysamples;
			...
			if (open_work.status2 != oldstatus2 || getpos2() >= 65536)
			{
				open_work.looped = copysamples != 0;
				break;
			}
		}
...
// 	} else if(IsEqualIID(riid, IID_IFILESTREAM)) {
// 		*ppvObject = (ISETFILEIO*)this;
// 	} else {
// 		*ppvObject = NULL;
