...
namespace rePlayer
{
    ...
    ReplayHighlyQuixotic::ReplayHighlyQuixotic(io::Stream* stream)
        : Replay(eExtension::Unknown, eReplay::HighlyQuixotic)
        , m_stream(stream)
    {}
    ...
    ReplayHighlyQuixotic* ReplayHighlyQuixotic::Load()
    {
        ...
        {
            if (psf_load(m_stream->GetName().c_str(), &m_psfFileSystem, 0x41, nullptr, nullptr, InfoMetaPSF, this, 0, nullptr, nullptr) >= 0)
            {
                auto extPos = m_stream->GetName().find_last_of('.');
                if (extPos == std::string::npos || _stricmp(m_stream->GetName().c_str() + extPos + 1, "qsflib") != 0)
                {
                    if (psf_load(m_stream->GetName().c_str(), &m_psfFileSystem, 0x41, QsoundLoad, this, nullptr, nullptr, 0, nullptr, nullptr) >= 0)
                        m_mediaType.ext = eExtension::_miniqsf;
                }
            }
        }
        else
    ...
    void ReplayHighlyQuixotic::ResetPlayback()
    {
        ...
        if (m_streamArchive.IsValid() && m_currentSubsongIndex != subsongIndex)
        {
            ...
            psf_load(m_stream->GetName().c_str(), &m_psfFileSystem, 0x41, QsoundLoad, this, InfoMetaPSF, this, 0, nullptr, nullptr);
