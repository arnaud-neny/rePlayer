...
int sc68_process(sc68_t * sc68, void * buf16st, int * _n)
{
  ...
  if (!is_sc68(sc68)) {
    ...
    //replayer begin : add break on loop
    if (ret & (SC68_LOOP | SC68_END | SC68_CHANGE)) /* exit on error|end|change */
        break;
    //replayer end
...
// rePlayer begin: we want to do some surround
int sc68_ym_channels(sc68_t* sc68, int channels)
{
    return ymio_active_channels(sc68->ymio, 7, channels);
}
// rePlayer end
