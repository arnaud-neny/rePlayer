...
std::pair<const module_impl::ctl_info *, const module_impl::ctl_info *> module_impl::get_ctl_infos() const {
	static constexpr ctl_info ctl_infos[] = {
		...
		{ "vblank", ctl_type::integer } // rePlayer
	};
...
void module_impl::ctl_set_integer( std::string_view ctl, std::int64_t value, bool throw_if_unknown ) {
	...
	// rePlayer begin
		...
		if (value >= -1ll && value <= 1ll && m_ctl_vblank != value) {
			if (m_ctl_oldvblank == -1)
				m_ctl_oldvblank = m_sndFile->m_playBehaviour[OpenMPT::kMODVBlankTiming];
			m_ctl_vblank = int(value);
			m_sndFile->m_playBehaviour.set(OpenMPT::kMODVBlankTiming, m_ctl_vblank == -1 ? bool(m_ctl_oldvblank) : bool(m_ctl_vblank));
		}
	// rePlayer end
	} else {
		MPT_ASSERT_NOTREACHED();
...
void module_impl::ctl_set_text( std::string_view ctl, std::string_view value, bool throw_if_unknown ) {
	...
	// rePlayer begin
		...
		if (value == "auto") {
			...
			throw openmpt::exception("invalid vblank type");
		}
		m_sndFile->m_playBehaviour.set(OpenMPT::kMODVBlankTiming, m_ctl_vblank == -1 ? bool(m_ctl_oldvblank) : bool(m_ctl_vblank));
	// rePlayer end
	} else {
