...
namespace
{
    void UpdateSampleDelta(uint32_t nexttime, uint32_t time, uint32_t usecs, uint32_t td2, uint32_t *smplrem, uint32_t *smpldelta)
    {
        ...
        uint64_t t = ((nexttime - time) * (uint64_t)usecs);
        uint64_t edx = t % (uint64_t)td2;
        uint64_t eax = t / (uint64_t)td2;
        edx += *smplrem;
        *smplrem = (uint32_t)(edx & 0xffFFffFF);
        *smpldelta += (uint32_t)(eax + (edx >> 32));
    }
}

V2MPlayer::V2MPlayer(bool useCore /* = true */, uint32_t a_tickspersec /* = 1000 */)
{
    m_tpc = a_tickspersec;
    /* m_base.valid=0; */
    memset(&m_base, 0, sizeof(V2MBase));

    // core doesn't play well with some tunes such as "we have accidently borrowed your votedisk"
    if (useCore) // fastest, less accurate
        m_synth = new uint8_t[synthGetSize()];
    else // slowest, accurate
        m_asmSynth = new V2::SynthAsm();
}
...
void V2MPlayer::Reset()
{
    ...
    if (m_samplerate)
    {
        if (m_asmSynth)
        {
            m_asmSynth->synthInit((void*)m_base.patchmap, m_samplerate);
            m_asmSynth->synthSetGlobals((void*)m_base.globals);
            synthSetLyrics(m_asmSynth->synthGetSpeechMem(), m_base.speechptrs);
        }
        else// if (m_synth)
        {
            synthInit(m_synth, (void*)m_base.patchmap, m_samplerate);
            synthSetGlobals(m_synth, (void*)m_base.globals);
            synthSetLyrics(synthGetSpeechMem(m_synth), m_base.speechptrs);
        }
    }
}
...
void V2MPlayer::Tick()
{
    ...
    *mptr ++= 0xfd;
    ...
    if (m_asmSynth)
        m_asmSynth->synthProcessMIDI(m_midibuf);
    else// if (m_synth)
...
void V2MPlayer::Render(float *a_buffer, uint32_t a_len, bool a_add)
{
    ...
    if (m_base.valid && m_state.state == PlayerState::PLAYING)
    {
        ...
        while (todo)
        {
            ...
            {
                if (m_asmSynth)
                    m_asmSynth->synthRender(a_buffer, torender, 0, a_add);
                else// if (m_synth)
    ...
    {
        if (m_asmSynth)
            m_asmSynth->synthRender(a_buffer, a_len, 0, a_add);
        else //if (m_synth)
            synthRender(m_synth, a_buffer, a_len, 0, a_add);
        m_state.cursmpl += a_len;
        m_extraSamples += a_len;
    }
...
{
    return m_base.valid && m_state.state != PlayerState::OFF && m_extraSamples < m_samplerate * 2;
}
...
uint32_t V2MPlayer::GetNumChannels() const
{
    uint32_t numChannels{ 0 };
    for (int ch = 0; ch < 16; ch++)
    {
        const V2MBase::Channel& bc = m_base.chan[ch];
        if (!bc.notenum)
            continue;
        numChannels++;
    }
