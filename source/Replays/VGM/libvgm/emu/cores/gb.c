...
static void gameboy_update(void *chip, UINT32 samples, DEV_SMPL **outputs)
{
	...
	for (i = 0; i < samples; i++)
	{
		...
		if (gb->snd[0].on && !gb->snd[0].Muted)
		{
			...
			if (gb->forceStereo)
			{
				if (gb->snd_control.chan_left[0] || gb->snd_control.chan_right[0])
					left += sample;
			}
			else
			{
		...
		if (gb->snd[1].on && !gb->snd[1].Muted)
		{
			...
			if (gb->forceStereo)
			{
				if (gb->snd_control.chan_left[1] || gb->snd_control.chan_right[1])
					left += sample;
			}
			else
			{
		...
		if (gb->snd[2].on && !gb->snd[2].Muted)
		{
			...
			if (gb->forceStereo)
			{
				if (gb->snd_control.chan_left[2] || gb->snd_control.chan_right[2])
					right += sample;
			}
			else
			{
		...
		if (gb->snd[3].on && !gb->snd[3].Muted)
		{
			...
			if (gb->forceStereo)
			{
				if (gb->snd_control.chan_left[3] || gb->snd_control.chan_right[3])
					right += sample;
			}
			else
			{
