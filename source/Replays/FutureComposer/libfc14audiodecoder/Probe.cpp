...
bool FC::isOurData(void *data, udword length) {
    ubyte *d = static_cast<ubyte*>(data);
    formatName = formatID = UNKNOWN_FORMAT_ID;
    offsets.header = 0xffffffff;
    // Check for Future Composer.
    if ( FC_detect(d,length) ) {
        return true;
    }
    // Search for Compressed TFMX.
    else if ( COSO_detect(d,length) ) {
        return true;
    }
    // Search for TFMX excluding COSO.
    else if ( TFMX_detect(d,length) ) {
        return true;
    }
    // Search for MCMD.
    else if ( MCMD_detect(d,length) ) {
        return true;
    }
    return false;
}
...
bool FC::TFMX_detect(ubyte* d, udword len) {
    bool maybe = false;
    ...
    if (maybe) {
        ...
        pInitFunc = &FC::TFMX_init;
        return true;
    }
    return false;
}

bool FC::TFMX_findTag(const ubyte* buf, udword len) {
...
bool FC::TFMX_findPortamentoCode() {
    bool foundIt = false;
    const ubyte* r;
    udword pos = 0;
    ...
    do {
        udword o = 0;
        r = std::search(input+pos,input+inputLen,portaCode,portaCode+sizeof(portaCode));
        if (r != (input+inputLen) ) {
            o = (udword)(r-input);
    ...
    while (r != (input+inputLen) && pos<inputLen);
    return foundIt;
}

bool FC::TFMX_7V_findPlayer(const ubyte* buf, udword len) {
...
bool FC::COSO_detect(ubyte* d, udword len) {
    bool maybe = false;
    ...
    if (maybe) {
        ...
        pInitFunc = &FC::COSO_init;
        return true;
    }
    return false;
}

bool FC::TFMX_COSO_findTags(const ubyte* buf, udword len) {
    const ubyte* r;
    udword pos = 0;
...
bool FC::COSO_findPlayer(const ubyte* buf, udword len) {
...
bool FC::findTag(const ubyte* buf, udword len, const std::string tag,
                 udword& location) {
...
bool FC::FC_detect(ubyte* d, udword len) {
...
bool FC::MCMD_detect(ubyte* d,udword len) {
    bool maybe = false;
    // Raw MCMD files.
