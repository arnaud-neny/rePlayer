...
const std::string FC::TFMX_7V_FORMAT_NAME = "TFMX 7V/Hippel (AMIGA)";
const std::string FC::TFMX_7V_ID = "TFMX7V";

// Switch from TFMX or COSO to 7V mode.
void FC::TFMX_7V_subInit() {
    stats.voices = 7;
    ...
    pChooseSongFunc = &FC::TFMX_7V_chooseSong;
    ...
    TFMX_sndModFuncs[10] = &FC::TFMX_sndSeq_EA;

    formatID = TFMX_7V_ID;
    formatName = TFMX_7V_FORMAT_NAME;
}

void FC::TFMX_7V_chooseSong(ubyte song) {
    udword offsetSongDef = offsets.subSongTab+song*TFMX_7V_SONGTAB_ENTRY_SIZE;
...
void FC::TFMX_7V_setRate(sbyte x) {
    setRate( (50*256)/(256+x) );  // [Hz]
}

// This is also used by COSO.
void FC::TFMX_7V_trackTabCmd(VoiceVars& voiceX, udword trackOffs) {
