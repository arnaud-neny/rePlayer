...
udword LamePaulaMixer::fillBuffer(void* buffer, udword bufferLen, PaulaPlayer *player) {
    // Both, 16-bit and stereo samples take more memory.
    // Hence fewer samples fit into the buffer.
    bufferLen >>= bufferScale;
    udword oldBufferLen = bufferLen;
    ...
    while ( bufferLen > 0 ) {
        ...
        if ( toFill == 0 && bufferLen != 0) {
            if (player->run(endCount == 1))
            {
                endCount = 0;
    ...
    return (oldBufferLen - bufferLen) << bufferScale;
}
