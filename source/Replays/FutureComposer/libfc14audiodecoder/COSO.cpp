...
const std::string FC::COSO_FORMAT_NAME = "Compressed TFMX/Hippel (AMIGA)";
const std::string FC::COSO_TAG = "COSO";

bool FC::COSO_init(int songNumber) {
    ...
    pNextNoteFunc = &FC::COSO_nextNote;
    ...
    if ( readBEudword(fcBuf,h+0xc)==0 ||
         ...
         readBEudword(fcBuf,h+0x18)>=inputLen ) {
        return false;
    }
...
void FC::COSO_nextNote(VoiceVars& voiceX) {
...
void FC::COSO_processPattern(VoiceVars& voiceX) {
