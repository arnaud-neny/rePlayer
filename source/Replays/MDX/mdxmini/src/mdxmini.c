...
*/
    ...
    /* load pdx data */
    pdx = data->pdx = _get_pdx( mdx, load_callback, user_data);
...
int mdx_calc_sample(t_mdxmini *data, short *buf, int buffer_size)
{
    ...
    sample_size = pcm8_get_sample_buffer_size(data->songdata);
	...
	do
	{
        ...
        if (calc_len > sample_size)
            calc_len = sample_size;
...
static PDX_DATA*
_open_pdx(char* name, pdx_load load_callback, void* user_data)
{
  unsigned char* buf = NULL;
  long length = 0;
  PDX_DATA* pdx = NULL;
  ...
  length = load_callback(name, &buf, user_data);
  if (!buf) {
    return NULL;
  }
...
static PDX_DATA*
_get_pdx(MDX_DATA* mdx, pdx_load load_callback, void* user_data)
{
  char *a = NULL;
  ...
  if ((pdx = _open_pdx(mdx->pdx_name, load_callback, user_data)) != NULL)
      goto get_pdx_file;
