...
MDX_DATA *mdx_open_mdx(void* buf_, size_t bufsize) {
...
*/
  mdx->data = malloc(bufsize + 16);
  memcpy(mdx->data, buf_, bufsize);
  memset(mdx->data + bufsize, 0, 16);
  mdx->length = bufsize;
  ...
  while(1) {
    ...
    if ( stricmp( ".pdx", (char *)(buf+ptr-1) )==0 ) j=1;
