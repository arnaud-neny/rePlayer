...
unsigned char* tfmx_get_next_buffer(TfmxState* state) {
...
static void filter(TfmxState* state, S32* buf, int nsamples) {
    int wl = state->audio_wl;
    int wr = state->audio_wr; /* actually backwards but who cares? */
    int i;
...
static void conv_float(TfmxState* state, S32* buf, int nsamples) {
    int i;
    S32* c = buf;
    float* a = (float*)&state->global_buf_union.b8[state->bhead];
    // register int l;
    ...
    for (i = 0; i < nsamples; i++) {
        // samples range are -16K,+16K (volume is max 64*64, sample -128;127, 2 tracks per channel)
        *a++ = buf[HALFBUFSIZE] / (32768.0f * 64 / 2);
        *a++ = *buf++ / (32768.0f * 64 / 2);
    }
    ...
    while ((r == 0) && ((state->bqueue + 2) < (BUFSIZE / (state->blocksize * state->bytes_per_sample)))) {
        ...
        while (nb > 0) {
            ...
            if (bd == state->blocksize) {
                conv_float(state, state->tbuf, bd);
                bd = 0;
                state->bqueue++;
                // printf("make %d\n",bqueue);
                r++;
