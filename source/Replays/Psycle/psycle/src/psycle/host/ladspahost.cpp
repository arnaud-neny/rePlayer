...
Machine* LadspaHost::LoadPlugin(const std::string& dllpath, int macIdx, int key) {
	...
	if (pfDescriptorFunction) {
		...
		several plugins*/
		const LADSPA_Descriptor* psDescriptor = pfDescriptorFunction(key); //key.index()); // ?
		if (psDescriptor) {
...
std::vector<PluginInfo> LadspaHost::LoadInfo(const std::string& fileName) { //const std::string& fullName, const std::string& fileName,PluginFinder& finder) {
	std::vector<PluginInfo> infos;
	#if defined __unix__ || defined __APPLE__
	#else
		if(fileName.find(".dll") == std::string::npos) return infos;
	#endif

	void* hInstance = LoadDll(fileName);
	if (!hInstance) return infos;
...
LADSPA_Descriptor_Function LadspaHost::LoadDescriptorFunction( void* libHandle_ ) {
...
LADSPA_Handle LadspaHost::Instantiate(const LADSPA_Descriptor* psDescriptor) {
...
void * LadspaHost::dlopenLADSPA(const char * pcFilename, int iFlag) {
