...
void filterS2::SetFilter_4PoleLP(int CurCutoff, int Resonance)
{
...
void filterS2::SetFilter_4PoleEQ1(int CurCutoff, int Resonance)
{
	float CutoffFreq=(float)(264*pow(32,CurCutoff/240.0));
	float cf=(float)CutoffFreq;
	if (cf>=sr/2) cf=sr/2; // próba wprowadzenia nieliniowoœci przy koñcu charakterystyki
...
void filterS2::SetFilter_4PoleEQ2(int CurCutoff, int Resonance)
{
	float CutoffFreq=(float)(264*pow(32,CurCutoff/240.0));
	float cf=(float)CutoffFreq;
	if (cf>=sr/2) cf=sr/2; // próba wprowadzenia nieliniowoœci przy koñcu charakterystyki
...
void filterS2::SetFilter_Vocal1(int CurCutoff, int Resonance)
{
...
void filterS2::SetFilter_Vocal2(int CurCutoff, int Resonance)
{
	float CutoffFreq=CurCutoff;
...
void filter::SetFilter_4PoleLP(int CurCutoff, int Resonance)
{
	...
	B=fB*ncf*ncf;
	a0=float(1/(1+A+B));
	Biquad2.m_b1=2*(Biquad2.m_b2=Biquad2.m_b0=0.35*a0);// obliczenie wspó³czynników filtru cyfrowego (przekszta³cenie dwuliniowe)
