...
{
  allpass->bufidx = 0;
  ALIGNED_MEMORY_ALLOC(16,fluid_real_t,allpass->buffer,size)
...
{
  ALIGNED_MEMORY_DEALLOC(fluid_real_t,allpass->buffer)
}
...
fluid_comb_setbuffer(fluid_comb* comb, int size)
{
  comb->filterstore = 0;
  comb->bufidx = 0;
  ALIGNED_MEMORY_ALLOC(16,fluid_real_t,comb->buffer,size)
  comb->bufsize = size;
}
...
{
  ALIGNED_MEMORY_DEALLOC(fluid_real_t,comb->buffer)
}
