...
#define GET_PTR(x) {unsigned long tmp; GET_WORD(tmp); if(tmp >= 0x8000) tmp=-0x10000+tmp; (x)=ptr-2+tmp;}
    ...
    if(*ptr == 'Z' && *(ptr + 1) == 'X' && *(ptr + 2) == 'A' && *(ptr + 3) == 'Y' && *(ptr + 4) == 'E' && *(ptr + 5) == 'M' && *(ptr + 6) == 'U' && *(ptr + 7) == 'L')
    {
        ...
        // rePlayer begin
        info.NumSongs = aydata.num_tracks;
        // rePlayer end
        ...
        if((aydata.tracks = new ayTrack[aydata.num_tracks]))
        {
            ...
            // rePlayer begin
            AY_initayfmt(info, aydata, info.CurrentSong);
            // rePlayer end
...
#define GET_PTR(x) {unsigned long tmp; GET_WORD(tmp); if(tmp >= 0x8000) tmp=-0x10000+tmp; (x)=ptr-2+tmp;}
    ...
    if(*ptr == 'Z' && *(ptr + 1) == 'X' && *(ptr + 2) == 'A' && *(ptr + 3) == 'Y' && *(ptr + 4) == 'E' && *(ptr + 5) == 'M' && *(ptr + 6) == 'U' && *(ptr + 7) == 'L')
    {
        ...
        if((aydata_loc.tracks = new ayTrack[aydata_loc.num_tracks]))
        {
            ...
            {
                // rePlayer begin
                info.Length = aydata_loc.tracks[info.CurrentSong].fadestart;
                if(!info.Length)
                    info.Length = 3000 * 3;
                info.TrackName = ay_sys_getstr(aydata_loc.tracks[info.CurrentSong].name, strlen((char *)aydata_loc.tracks[info.CurrentSong].name));
                // rePlayer end
