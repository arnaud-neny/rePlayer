...
unsigned opna_timer_mix_tick(struct opna_timer *timer, int16_t *buf, unsigned maxsamples) {
  unsigned generate_samples = maxsamples;
  if (timer->timerb_enable && timer->timerb_load) {
    unsigned timerb_samples = (1<<TIMERB_BITS) - timer->timerb_cnt;
    if (timerb_samples < generate_samples) {
      generate_samples = timerb_samples;
  ...
  opna_mix_oscillo(timer->opna, buf, generate_samples, 0);
  if (timer->mix_cb) {
    timer->mix_cb(timer->mix_userptr, buf, generate_samples);
  }
  if (timer->timera_load) {
...
unsigned opna_timer_tick(struct opna_timer* timer) {
  unsigned generate_samples = 0;
  if (timer->timerb_enable && timer->timerb_load) {
    generate_samples = (1<<TIMERB_BITS) - timer->timerb_cnt;
  }
  if (timer->timera_enable && timer->timera_load) {
    unsigned timera_samples = (1<<TIMERA_BITS) - timer->timera;
    if (generate_samples == 0 || timera_samples < generate_samples) {
      generate_samples = (1 << TIMERA_BITS) - timer->timera;
    }
  }
  if (timer->timera_load) {
    timer->timera = (timer->timera + generate_samples) & ((1<<TIMERA_BITS)-1);
