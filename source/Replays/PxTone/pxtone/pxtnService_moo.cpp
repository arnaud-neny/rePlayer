...
bool pxtnService::_moo_PXTONE_SAMPLE( void *p_data )
{
	...
	_moo_b_looped = _moo_smp_count >= _moo_smp_end; // rePlayer
...
int32_t pxtnService::Moo( void* p_buf, int32_t  size ) // rePlayer
{
	if( !_moo_b_init       ) return false;
...

	{
		...
		for( smp_w = 0; smp_w < smp_num; smp_w++ )
		{
			...
			if (_moo_b_looped) { return smp_w; } // rePlayer
