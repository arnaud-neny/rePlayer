...
#define GdMalloc(size)                  smem_new((size))
#define GdCalloc(num,size)              smem_znew((num)*(size))
#define GdRealloc(addr,oldsize,newsize) smem_resize((addr),(newsize))
#define GdFree(addr)                    smem_free((addr))
#define GdStrDup(string)                smem_strdup((string))
