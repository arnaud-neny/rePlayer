...
ogg_reference *tremor_ogg_buffer_dup(ogg_reference *or){
  ogg_reference *ret=0,*head=0;
  /* duplicate the reference chain; increment refcounts */
  while(or){
    ogg_reference *temp=_fetch_ref(or->buffer->ptr.owner);
...
ogg_int64_t tremor_ogg_page_granulepos(tremor_ogg_page *og){
  oggbyte_buffer ob;
  if(oggbyte_init(&ob,og->header))return -1;
  return oggbyte_read8(&ob,6);
}

ogg_uint32_t tremor_ogg_page_serialno(tremor_ogg_page *og){
  oggbyte_buffer ob;
  if(oggbyte_init(&ob,og->header)) return 0xffffffffUL;
  return oggbyte_read4(&ob,14);
}
 
ogg_uint32_t tremor_ogg_page_pageno(tremor_ogg_page *og){
  oggbyte_buffer ob;
  if(oggbyte_init(&ob,og->header))return 0xffffffffUL;
...
tremor_ogg_sync_state *tremor_ogg_sync_create(void){
  tremor_ogg_sync_state *oy=_ogg_calloc(1,sizeof(*oy));
...
tremor_ogg_stream_state *tremor_ogg_stream_create(int serialno){
  tremor_ogg_stream_state *os=_ogg_calloc(1,sizeof(*os));
