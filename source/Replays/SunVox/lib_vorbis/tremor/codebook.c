...
void tremor_vorbis_book_clear(codebook *b){
...
int tremor_vorbis_book_unpack(tremor_oggpack_buffer *opb,codebook *s){
...
long tremor_vorbis_book_decode(codebook *book, tremor_oggpack_buffer *b){
  if(book->dec_type)return -1;
 return decode_packed_entry_number(book,b);
}

int decode_map(codebook *s, tremor_oggpack_buffer *b, ogg_int32_t *v, int point){
  ...
  case 3:{
    /* offset into array */
    void *ptr=(unsigned char *)(s->q_val)+entry*s->q_pack; // rePlayer
...
long tremor_vorbis_book_decodevs_add(codebook *book,ogg_int32_t *a,
...
long tremor_vorbis_book_decodev_add(codebook *book,ogg_int32_t *a,
			     tremor_oggpack_buffer *b,int n,int point){
  if(book->used_entries>0){
    ogg_int32_t *v = (ogg_int32_t *)alloca(sizeof(*v)*book->dim);
    int i,j;
...
long tremor_vorbis_book_decodev_set(codebook *book,ogg_int32_t *a,
			     tremor_oggpack_buffer *b,int n,int point){
  if(book->used_entries>0){
    ogg_int32_t *v = (ogg_int32_t *)alloca(sizeof(*v)*book->dim);
    int i,j;
...
long tremor_vorbis_book_decodevv_add(codebook *book,ogg_int32_t **a,
			      long offset,int ch,
			      tremor_oggpack_buffer *b,int n,int point){
  if(book->used_entries>0){
