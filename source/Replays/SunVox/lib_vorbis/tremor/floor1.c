...
void floor1_free_info(tremor_vorbis_info_floor *i){
  tremor_vorbis_info_floor1 *info=(tremor_vorbis_info_floor1 *)i;
  if(info){
...
tremor_vorbis_info_floor *floor1_info_unpack (tremor_vorbis_info *vi,tremor_oggpack_buffer *opb){
  codec_setup_info     *ci=(codec_setup_info *)vi->codec_setup;
  int j,k,count=0,maxclass=-1,rangebits;
  
  tremor_vorbis_info_floor1 *info=(tremor_vorbis_info_floor1 *)_ogg_calloc(1,sizeof(*info));
  /* read partitions */
...
int floor1_memosize(tremor_vorbis_info_floor *i){
  tremor_vorbis_info_floor1 *info=(tremor_vorbis_info_floor1 *)i;
  return info->posts;
}
...
ogg_int32_t *floor1_inverse1(tremor_vorbis_dsp_state *vd,tremor_vorbis_info_floor *in,
			     ogg_int32_t *fit_value){
  tremor_vorbis_info_floor1 *info=(tremor_vorbis_info_floor1 *)in;
  ...
  if(tremor_oggpack_read(&vd->opb,1)==1){
    ...
    for(i=0,j=2;i<info->partitions;i++){
      ...
      if(csubbits){
	cval=tremor_vorbis_book_decode(books+info->class[classv].class_book,&vd->opb);
	...
	cval>>=csubbits;
	if(book!=0xff){
	  if((fit_value[j+k]=tremor_vorbis_book_decode(books+book,&vd->opb))==-1)
	    goto eop;
	}else{
	  fit_value[j+k]=0;
	}
...
int floor1_inverse2(tremor_vorbis_dsp_state *vd,tremor_vorbis_info_floor *in,
