...
struct hvl_tune *hvl_load_ahx( const uint8 *buf, uint32 buflen, uint32 defstereo, uint32 freq, hvl_malloc_callback mallocCb, hvl_free_callback freeCb )
{
  ...
  strncpy_s( ht->ht_Name, sizeof(ht->ht_Name), (TEXT *)&buf[(buf[4]<<8)|buf[5]], 128 );
  ...
  for( i=1; i<=ht->ht_InstrumentNr; i++ )
  {
    ...
    {
      strncpy_s( ht->ht_Instruments[i].ins_Name, sizeof(ht->ht_Instruments[i].ins_Name), nptr, 128 );
      nptr += strlen( nptr )+1;
    } else {
