...
void ExternalFilter::setClockFrequency(double frequency)
{
    ...
    w0lp_1_s7[0] = w0lp_1_s7[1] = static_cast<int>((dt / (dt + getRC(10e3, 1000e-12))) * (1 << 7) + 0.5);

    // High-pass: R = 10kOhm, C = 10uF;   w0h = dt/(dt+RC) = 1e-6/(1e-6+1e4*1e-5) = 0.00000999
    // Cutoff 1/2*PI*RC = 1/2*PI*1e4*1e-5 = 1.59155 Hz
    w0hp_1_s17[0] = w0hp_1_s17[1] = static_cast<int>((dt / (dt + getRC(10e3, 10e-6))) * (1 << 17) + 0.5);
}

void ExternalFilter::reset()
{
    // State of filter.
    Vlp[0] = Vlp[1] = 0; //1 << (15 + 11);
    Vhp[0] = Vhp[1] = 0;
}
