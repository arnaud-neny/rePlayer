...
SampleI32 Filter::clock(SampleI32 FilterInput, SampleI32 NonFiltered)
{
	...
	for (int i = 0; i < 2; ++i)
	{
		...
		{ // 6581
			...
			Cutoff += ((&FilterInput.left)[i] * 105) >> 16;
		...
		int FilterOutput = 0;
		{
			int Tmp = (&FilterInput.left)[i] + ((PrevBandPass[i] * Resonance) / CRSID_FILTERTABLE_MAGNITUDE) + PrevLowPass[i];
			if (VolumeBand & HIGHPASS_BITVAL)
				FilterOutput -= Tmp;
			Tmp = PrevBandPass[i] - ((Tmp * Cutoff) / CRSID_FILTERTABLE_MAGNITUDE);
			PrevBandPass[i] = Tmp;
			if (VolumeBand & BANDPASS_BITVAL)
				FilterOutput -= Tmp;
			Tmp = PrevLowPass[i] + ((Tmp * Cutoff) / CRSID_FILTERTABLE_MAGNITUDE);
			PrevLowPass[i] = Tmp;
		...
		if (LIKELY(s->getRealSIDmode()))
		{
			...
			Digi = (Tmp - PrevVolume[i]) * D418_DIGI_MUL;
			// arithmetic shift amount determines digi lowpass-frequency
			PrevVolume[i] += (Tmp - PrevVolume[i]) / 1024;
			// lowpass is main volume
			MainVolume = PrevVolume[i] >> FRACTIONAL_SHIFTS;
		}
		else
			MainVolume = VolumeBand & 0xF;

		(&Output.left)[i] = (((&NonFiltered.left)[i] + FilterOutput) * MainVolume) + Digi;// / ((CHANNELS*VOLUME_MAX) + Attenuation);

		if (UNLIKELY(!++VUmeterUpdateCounter[i]))
		{
			// average level (for VU-meter)
			Level[i] += ((std::abs((&Output.left)[i]) >> VUMETER_DIVSHIFTS) - Level[i]) / VUMETER_LOWPASS_DIV;
		}
		(&Output.left)[i] /= Attenuation;
	}
...
void Filter::reset()
{
	for (int i = 0; i < 2; ++i)
	{
		PrevLowPass[i] = PrevBandPass[i] = PrevVolume[i] = 0;
		Level[i] = 0;
		VUmeterUpdateCounter[i] = 0;
	}
}
