...
namespace rePlayer
{
    ReplayPlugin g_replayPlugin = {
        .replayId = eReplay::MVX,
        .name = "Conspiracy's MVX Synth",
        .extensions = "mvm",
        .about = "Conspiracy's MVX Synth\nCopyright (c) 2010-2023 Gargaj",
        .load = ReplayMVX::Load
    };

    Replay* ReplayMVX::Load(io::Stream* stream, CommandBuffer /*metadata*/)
    {
        // check header
        const char magic[] = { 'M', 'V', 'M', 0 };
        ...
        {
            auto path = std::filesystem::path(pgrPath).remove_filename() / "replays" REPLAYER_OS_STUB / "MVX\\MVXBridge.exe";
            path = path.lexically_normal();
            bridgePath = path.generic_string();
        }
        auto* bridgePathStr = bridgePath.c_str();
#else
        const char* bridgePathStr = "replays" REPLAYER_OS_STUB "\\MVX\\MVXBridge.exe";
#endif
        ...
        if (CreateProcessA(bridgePathStr, id, nullptr, nullptr, TRUE, 0, nullptr, nullptr, &si, &pi))
        {
            ...
            if (sharedMemory->hasFailed)
                return nullptr;
            return new ReplayMVX(pi.hProcess, pi.hThread, hFileMapping.Detach(), hRequestEvent.Detach(), hResponseEvent.Detach(), sharedMemory.Detach());
        }
    ...
    ReplayMVX::ReplayMVX(void* hProcess, void* hThread, void* hFileMapping, void* hRequestEvent, void* hResponseEvent, SharedMemory* sharedMemory)
        : Replay(eExtension::_mvm, eReplay::MVX)
        , m_hProcess(hProcess)
        , m_hThread(hThread)
    ...
    uint32_t ReplayMVX::Render(StereoSample* output, uint32_t numSamples)
    {
        ...
        while (remainingSamples)
        {
            if (mNumSamples == 0)
            {
                ...
                // stability issues with MVX
                if (WaitForSingleObject(m_hResponseEvent, 1000) != WAIT_OBJECT_0)
                    break;
    ...
    void ReplayMVX::ApplySettings(const CommandBuffer /*metadata*/)
    {}

    void ReplayMVX::SetSubsong(uint32_t)
    {}

    uint32_t ReplayMVX::GetDurationMs() const
    {
        return m_sharedMemory->songDuration;
    }

    uint32_t ReplayMVX::GetNumSubsongs() const
    {
        return 1;
    }

    std::string ReplayMVX::GetExtraInfo() const
    {
        return {};
    }

    std::string ReplayMVX::GetInfo() const
    {
        std::string info;
        info = "2 channels\nMVM: ";
        char buf[32];
        sprintf(buf, "%u machine%c", m_sharedMemory->numMachines, m_sharedMemory->numMachines > 1 ? 's' : ' ');
        info += buf;
        info += "\nConspiracy's MVX Synth";
        return info;
    }
}
// namespace rePlayer