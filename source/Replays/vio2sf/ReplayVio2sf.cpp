...
namespace rePlayer
{
    ...
    ReplayVio2sf* ReplayVio2sf::Load(CommandBuffer metadata)
    {
        ...
        if (archive_read_open_memory(archive, data.Items(), data.Size()) != ARCHIVE_OK)
        {
            ...
            if (psf_load(m_stream->GetName().c_str(), &m_psfFileSystem, 0x24, nullptr, nullptr, InfoMetaPSF, this, 0, nullptr, nullptr) >= 0)
            {
                auto extPos = m_stream->GetName().find_last_of('.');
                if (extPos == std::string::npos || _stricmp(m_stream->GetName().c_str() + extPos + 1, "2sflib") != 0)
                {
    ...
    void ReplayVio2sf::ResetPlayback()
    {
        ...
        if (m_streamArchive.IsValid() && m_currentSubsongIndex != subsongIndex)
        {
            ...
            psf_load(m_stream->GetName().c_str(), &m_psfFileSystem, 0x24, TwosfLoad, this, InfoMetaPSF, this, 0, nullptr, nullptr);
