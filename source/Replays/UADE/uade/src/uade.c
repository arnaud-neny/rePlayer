...
inline uint32_t ntohl(uint32_t a) { return _byteswap_ulong(a); }
inline uint32_t htonl(uint32_t a) { return _byteswap_ulong(a); }
...
void uadecore_check_sound_buffers(int bytes)
{
  ...
  if (uade_send_message(um, &uadecore_ipc)) {
    assert(0 && "uadecore: Could not send sample data");
  }
  ...
  if (uadecore_read_size == 0) {
    ...
    if (uade_send_short_message(UADE_COMMAND_TOKEN, &uadecore_ipc)) {
      assert(0 && "uadecore: Could not send token (after samples)");
    }
...
void uadecore_get_amiga_message(void)
{
	...
	case AMIGAMSG_SUBSINFO:
		...
		u32ptr[2] = htonl(curs);
		if (uade_send_message(um, &uadecore_ipc)) {
			assert(0 && "uadecore: Could not send subsong info message");
		}
		break;
...
void uadecore_handle_r_state(void)
{
  ...
  while (1) {
    ...
    if (ret == 0) {
      ...
      assert(0 && "uadecore: Error on input. Exiting with error");
      break;
    }
	...
	assert(0 && "uadecore: Invalid size with change subsong");
	break;
      }
      change_subsong(x);
      break;
	...
	assert(0 && "uadecore: Invalid size with filter command");
	break;
      }
    ...
    case UADE_COMMAND_SET_FREQUENCY:
      if (uade_parse_u32_message(&x, um)) {
	assert(0 && "Invalid frequency message size");
	break;
      }
      set_sound_freq(x);
      break;
    ...
    case UADE_COMMAND_READ:
      if (uadecore_read_size != 0) {
	assert(0 && "uadecore: Read not allowed when uadecore_read_size > 0");
	break;
      }
      if (uade_parse_u32_message(&x, um)) {
	assert("uadecore: Invalid size on read command");
	break;
      }
	  if (x == 0 || x > MAX_SOUND_BUF_SIZE || (x & 3) != 0) {
	assert(0 && "uadecore: Invalid read size");
	break;
      }
    ...
    case UADE_COMMAND_SET_SUBSONG:
      if (uade_parse_u32_message(&x, um)) {
	assert(0 && "uadecore: Invalid size on set subsong command");
	break;
      }
    ...
    default:
      assert(0 && "uadecore: Received invalid command");
      break;
    }
  }
}
...
void uadecore_option(int argc, char **argv)
{
  ...
  if (!s_argv) {
    assert(0 && "uadecore: Out of memory for command line parsing");
    return;
  }
  s_argc = 0;
  ...
  for (i = 1; i < argc;) {
	...
	if ((i + 1) >= argc) {
	  assert(0 && "uadecore: parameter missing");
	...
	if (in_fd < 0 || *endptr != 0) {
		assert(0 && "uadecore: Invalid -i parameter");
        free(s_argv);
	}
	i += 2;
	...
	if ((i + 1) >= argc) {
	  assert("uadecore: %s parameter missing");
	  uade_print_help(OPTION_ILLEGAL_PARAMETERS, argv[0]);
      free(s_argv);
	  return;
	}
	...
	if (out_fd < 0 || *endptr != 0) {
		assert(0 && "uadecore: Invalid -o parameter");
        free(s_argv);
		return;
	}
	i += 2;
  ...
  if (in_fd < 0 || out_fd < 0) {
	  assert(0 && "uadecore: Must have -i and -o parameters");
      free(s_argv);
	  return;
  }
  ...
  if (ret == 0) {
    assert(0 && "uadecore: No config file passed as a message");
    free(s_argv);
    return;
  } else if (ret < 0) {
    assert(0 && "uadecore: Invalid input. Expected a config file");
    free(s_argv);
    return;
  }
  ...
  if (!cfg_loaded) {
    if (cfgfile_load (&currprefs, optionsfile) == 0) {
      assert(0 && "uadecore: Could not load uaerc");
      return;
    }
  }
...
void uadecore_reset(void)
{
 ...
 nextsong:
  ...
  if (highmem < 0x80000) {
    assert(0 && "uadecore: There must be at least 512 KiB of amiga memory");
	quit_program = 1;
    return;
  }
  if (highmem < 0x200000) {
  ...
  if (ret == 0) {
    ...
    assert(0 && "uadecore: Invalid input. Expected score name");
    quit_program = 1;
    return;
  }
  ...
  if (player == NULL || player->data == NULL) {
	  assert(0 && "uadecore: Invalid input. Expected player");
      quit_program = 1;
      return;
  }
  if (player->name == NULL) {
  ...
  if (module == NULL) {
	  assert(0 && "uadecore: Invalid input. Expected module");
      quit_program = 1;
      return;
  }
  if (module->name != NULL)
  ...
  if (uade_receive_short_message(UADE_COMMAND_TOKEN, &uadecore_ipc)) {
    assert(0 && "uadecore: Can not receive token in uade_reset().\n");
    quit_program = 1;
    return;
  }

  if (uade_send_short_message(UADE_REPLY_CAN_PLAY, &uadecore_ipc)) {
    assert(0 && "uadecore: Can not send 'CAN_PLAY' reply");
    quit_program = 1;
    return;
  }
  if (uade_send_short_message(UADE_COMMAND_TOKEN, &uadecore_ipc)) {
    assert("uadecore: Can not send token from uade_reset()");
    quit_program = 1;
    return;
  }
 ...
 skiptonextsong:
  ...
  if (uade_send_short_message(UADE_REPLY_CANT_PLAY, &uadecore_ipc)) {
    assert(0 && "uadecore: Can not send 'CANT_PLAY' reply");
    quit_program = 1;
    return;
  }
  if (uade_send_short_message(UADE_COMMAND_TOKEN, &uadecore_ipc)) {
    assert(0 && "uadecore: Can not send token from uade_reset()");
    quit_program = 1;
    return;
  }
  goto nextsong;
...
void uadecore_song_end(char *reason, int kill_it)
{
  ...
  if (uade_send_message(um, &uadecore_ipc)) {
    assert(0 && "uadecore: Could not send song end message");
    quit_program = 1;
    return;
  }
	/*
