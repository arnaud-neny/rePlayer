...
inline uint16_t ntohs(uint16_t a) { return _byteswap_ushort(a); }
...
void uade_cleanup_state(struct uade_state *state)
{
	...
	uade_arch_kill_and_wait_uadecore(&state->ipc, &state->userdata);
...
{
/* we don't want to change the subsong, so disable this code
	...
	while (1) {
		...
		case UADE_STATE_SONG_END_PENDING:
			...
			*event = state->song.endevent;
			state->song.state = UADE_STATE_RECEIVE_MSGS; // complete the event instead of asking for the next subsong
			return 0;
...
ssize_t uade_read(void *_data, size_t bytes, struct uade_state *state)
{
	...
	while (copied < bytes) {
		...
		case UADE_EVENT_SONG_END:
			...
			{
				// update the end flag for the replay
				state->hasended = 1;
				return copied;
			}
...
*/
/* rePlayer: don't need to test this
...
*/
	...
	if (uade_arch_spawn(&state->ipc, &state->userdata,
				state->config.uadecore_file.name)) {
