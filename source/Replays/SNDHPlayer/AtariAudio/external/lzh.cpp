...
static uint32_t	ReadLE32(const uint8_t* r)
{
	uint32_t v = r[0];
	v |= r[1]<<8;
	v |= r[2]<<16;
...
void*	LzhDepacker::Unpack(const void* dataIn, uint32_t inSize, uint32_t& outSize)
{
	void* ret = nullptr;
	outSize = 0;
...
int		LzhDepacker::DataIn(void *pBuffer,int nBytes)
{
...
int		LzhDepacker::DataOut(void *pBuffer,int nBytes)
{
...
void LzhDepacker::fillbuf (int n)
{
...
ushort LzhDepacker::getbits (int n)
{
    ushort x;
...
void LzhDepacker::init_getbits (void)
{
    bitbuf = 0;
    subbitbuf = 0;
    bitcount = 0;
...
int LzhDepacker::make_table (int nchar, uchar *bitlen,
...
void LzhDepacker::read_pt_len (int nn, int nbit, int i_special)
{
    int i, n;
    short c;
    ushort mask;
...
void LzhDepacker::read_c_len (void)
{
    short i, c, n;
    ushort mask;
...
ushort LzhDepacker::decode_c (void)
{
    ushort j, mask;
...
ushort LzhDepacker::decode_p (void)
{
    ushort j, mask;
...
void LzhDepacker::huf_decode_start (void)
{
    init_getbits ();
    blocksize = 0;
}
...
void LzhDepacker::decode_start (void)
{
    fillbufsize = 0;
    huf_decode_start ();
    decode_j = 0;
...
*/
void LzhDepacker::decode (uint count, uchar buffer[])
{
    uint r, c;
...
bool	LzhDepacker::LzUnpack(void *pSrc,int srcSize,void *pDst,int dstSize)
{
