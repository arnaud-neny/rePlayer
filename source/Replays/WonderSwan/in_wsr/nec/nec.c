...
static void nec_interrupt(unsigned int_num,bool md_flag)
{
    uint32_t dest_seg, dest_off;
...
OP( 0x0f, i_pre_nec  ) { uint32_t ModRM, tmp, tmp2; /* pop cs at V30MZ? */
	switch (FETCH) {
...
OP( 0x69, i_imul_d16 ) { uint32_t tmp;	DEF_r16w; 	FETCHWORD(tmp); dst = (int32_t)((int16_t)src)*(int32_t)((int16_t)tmp); I.CarryVal = I.OverVal = (((int32_t)dst) >> 15 != 0) && (((int32_t)dst) >> 15 != -1);     RegWord(ModRM)=(uint16_t)dst;     CLKM(4,3);}
...
OP( 0x9a, i_call_far  ) { uint32_t tmp, tmp2;	FETCHWORD(tmp); FETCHWORD(tmp2); PUSH(I.sregs[CS]); PUSH(I.ip); I.ip = (uint16_t)tmp; I.sregs[CS] = (uint16_t)tmp2; CLK(10); }
OP( 0x9b, i_wait      ) { ; }
OP( 0x9c, i_pushf     ) { PUSH( CompressFlags() ); CLK(2); }
OP( 0x9d, i_popf      ) { uint32_t tmp; POP(tmp); ExpandFlags(tmp); CLK(3); nec_int(); }
OP( 0x9e, i_sahf      ) { uint32_t tmp = (CompressFlags() & 0xff00) | (I.regs.b[AH] & 0xd5); ExpandFlags(tmp); CLK(4); nec_int(); }
OP( 0x9f, i_lahf      ) { I.regs.b[AH] = CompressFlags() & 0xff; CLK(2); }

OP( 0xa0, i_mov_aldisp ) { uint32_t addr; FETCHWORD(addr); I.regs.b[AL] = GetMemB(DS, addr); CLK(1); }
OP( 0xa1, i_mov_axdisp ) { uint32_t addr; FETCHWORD(addr); I.regs.b[AL] = GetMemB(DS, addr); I.regs.b[AH] = GetMemB(DS, (addr+1)&0xffff); CLK(1); }
OP( 0xa2, i_mov_dispal ) { uint32_t addr; FETCHWORD(addr); PutMemB(DS, addr, I.regs.b[AL]);  CLK(1); }
OP( 0xa3, i_mov_dispax ) { uint32_t addr; FETCHWORD(addr); PutMemB(DS, addr, I.regs.b[AL]);  PutMemB(DS, (addr+1)&0xffff, I.regs.b[AH]); CLK(1); }
...
OP( 0xd4, i_aam    ) { uint32_t mult=FETCH; mult=0; I.regs.b[AH] = I.regs.b[AL] / 10; I.regs.b[AL] %= 10; SetSZPF_Word(I.regs.w[AW]); CLK(17); }
OP( 0xd5, i_aad    ) { uint32_t mult=FETCH; mult=0; I.regs.b[AL] = I.regs.b[AH] * 10 + I.regs.b[AL]; I.regs.b[AH] = 0; SetSZPF_Byte(I.regs.b[AL]); CLK(6); }
...
OP( 0xe8, i_call_d16 ) { uint32_t tmp; FETCHWORD(tmp); PUSH(I.ip); I.ip = (uint16_t)(I.ip+(int16_t)tmp); CLK(5); }
OP( 0xe9, i_jmp_d16  ) { uint32_t tmp; FETCHWORD(tmp); I.ip = (uint16_t)(I.ip+(int16_t)tmp); CLK(4); }
OP( 0xea, i_jmp_far  ) { uint32_t tmp,tmp1; FETCHWORD(tmp); FETCHWORD(tmp1); I.sregs[CS] = (uint16_t)tmp1; 	I.ip = (uint16_t)tmp; CLK(7);  }
