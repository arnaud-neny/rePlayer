...
Blip_Synth_::Blip_Synth_( short* p, int w , int b) :
	impulses( p ),
	width( w ),
	blip_phase_bit(b),
	blip_res(1<<b)
...
void blip_eq_t::generate( float* out, int count , int blip_res) const
{
...
{
	float* fimpulse = (float*)malloc((blip_res / 2 * (blip_widest_impulse_ - 1) + blip_res * 2)*sizeof(float));
	assert(fimpulse);
	int const half_size = blip_res / 2 * (width - 1);
	eq.generate( fimpulse+blip_res, half_size, blip_res );
