...
#define DEFAULT_BLIP_PHASE_BITS 8
...
//#ifndef DEFAULT_BLIP_PHASE_BITS
//	#if BLIP_BUFFER_FAST
//		#define DEFAULT_BLIP_PHASE_BITS 8
//	#else
//		#define DEFAULT_BLIP_PHASE_BITS 6
//	#endif
//#endif
	...
	public:
		...
		void volume_unit( double );
		Blip_Synth_( short* impulses, int width , int blip_phase_bit);
		void treble_eq( blip_eq_t const& );
	private:
		double volume_unit_;
		short* const impulses;
		int const width;
		int const blip_phase_bit;
		int const blip_res;
...
// amplitudes (max - min).
template<int quality,int range,int blip_phase_bit=DEFAULT_BLIP_PHASE_BITS>
class Blip_Synth {
public:
...
public:
	Blip_Synth() : blip_res(1<<blip_phase_bit) {}
#else
	Blip_Synth_ impl;
	typedef short imp_t;
	imp_t impulses [(1<<blip_phase_bit) * (quality / 2) + 1];
public:
	Blip_Synth() : blip_res(1<<blip_phase_bit), impl( impulses, quality, blip_phase_bit ) { }
#endif
};
...
private:
	...
	long sample_rate;
	long cutoff_freq;
	void generate( float* out, int count, int blip_res ) const;
...
template<int quality,int range,int blip_phase_bit>
blip_inline void Blip_Synth<quality,range,blip_phase_bit>::offset_resampled( blip_resampled_time_t time,
		int delta, Blip_Buffer* blip_buf ) const
{
	...
	int phase = (int) (time >> (BLIP_BUFFER_ACCURACY - blip_phase_bit) & (blip_res - 1));
	...
	blip_long right = (delta >> blip_phase_bit) * phase;
	left  -= right;
	right += buf [1];
...
template<int quality,int range,int blip_phase_bit>
	...
	blip_inline
#endif
void Blip_Synth<quality,range,blip_phase_bit>::offset( blip_time_t t, int delta, Blip_Buffer* buf ) const
{
	offset_resampled( t * buf->factor_ + buf->offset_, delta, buf );
}
...
template<int quality,int range,int blip_phase_bit>
	...
	blip_inline
#endif
void Blip_Synth<quality,range,blip_phase_bit>::update( blip_time_t t, int amp )
{
