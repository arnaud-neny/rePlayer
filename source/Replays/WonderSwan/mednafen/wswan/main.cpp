...
#endif
{
 try
 {
...
#else
  memcpy(wsCartROM + (rom_size - real_rom_size), pData, fp_in_size);
#endif
  ...
  MDFN_printf(_("ROM MD5:   0x%s\n"), md5_context::asciistr(MDFNGameInfo->MD5, 0).c_str());
   ...
   throw MDFN_Error(0, _("This is not WSR."));
#endif
  uint8 header[10];
...
#else
  WSwan_MemoryInit(false, wsc, SRAMSize, IsWW);
#endif
#ifdef WSRP_REMOVED
  if(!IsWSR)
   WSwan_MemoryLoadNV();
