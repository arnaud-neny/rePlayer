...
namespace rePlayer
{
    ...
    void Library::DatabaseUI<ParentDatabaseUI>::SongMerger::MenuItem(DatabaseUI<ParentDatabaseUI>& songs)
    {
        if (++m_frame != ImGui::GetFrameCount())
        {
            ...
            {
                if (entry.IsSelected() && m_entries.Find(entry.songId) == nullptr)
                    m_entries.Add({ songs.m_db[entry] });
            }
        }
    ...
    template <typename ParentDatabaseUI>
    void Library::DatabaseUI<ParentDatabaseUI>::SongMerger::Update(DatabaseUI<ParentDatabaseUI>& songs)
    {
        if (m_isStarted)
        {
            ...
            if (ImGui::BeginTable("Merge", 8, ImGuiTableFlags_Resizable | ImGuiTableFlags_SizingFixedFit | ImGuiTableFlags_BordersInnerV | ImGuiTableFlags_RowBg | ImGuiTableFlags_ScrollY, ImVec2(0, -ImGui::GetFrameHeightWithSpacing()))) // -GetFrameHeightWithSpacing to display the merge button
            {
                ...
                while (clipper.Step())
                {
                    ...
                    for (int rowIdx = clipper.DisplayStart; rowIdx < clipper.DisplayEnd; rowIdx++)
                    {
                        ...
                        if (ImGui::Selectable("##selected", m_entries[rowIdx].isSelected, ImGuiSelectableFlags_SpanAllColumns | ImGuiSelectableFlags_AllowOverlap | ImGuiSelectableFlags_AllowDoubleClick | ImGuiSelectableFlags_NoAutoClosePopups))
                        {
                            if (ImGui::IsMouseDoubleClicked(0))
                            {
                                if (ImGui::GetIO().KeyCtrl)
                                {
                                    for (uint16_t i = 0; i <= song->GetLastSubsongIndex(); i++)
                                    {
                                        ...
                                        {
                                            Core::GetDeck().PlaySolo(Core::GetLibrary().LoadSong({SubsongID(song->GetId(), i), DatabaseID::kLibrary}));
                                            break;
                                        }
                                    }
                                }
    ...
    template <typename ParentDatabaseUI>
    void Library::DatabaseUI<ParentDatabaseUI>::SongMerger::Unmerge(int32_t unmergedEntryIndex)
    {
    ...
    template <typename ParentDatabaseUI>
    void Library::DatabaseUI<ParentDatabaseUI>::SongMerger::Remerge(int32_t rootEntryIndex)
    {
        // lazy code here...
    ...
    template <typename ParentDatabaseUI>
    void Library::DatabaseUI<ParentDatabaseUI>::SongMerger::Process(DatabaseUI<ParentDatabaseUI>& songs)
    {
